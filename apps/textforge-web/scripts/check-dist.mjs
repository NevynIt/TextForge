...
const [distIndexHtml, distScriptJs, distStyleCss, distDocsJs, scriptLoaderSource, viteConfigSource] = await Promise.all([
  ...
  readFile(resolve(distDir, 'assets/textforge-bundled-docs.js'), 'utf8'),
...
if (!distIndexHtml.includes('<script defer src="./assets/textforge-bundled-docs.js"></script>')) {
  throw new Error('dist/index.html must load the optional bundled docs payload as a separate file');
}
...
if (!distDocsJs.includes('globalThis.TextForgeBundledDocs')) {
  throw new Error('dist/assets/textforge-bundled-docs.js must provide the optional docs global payload');
}

if (distScriptJs.includes('Generated by scripts/sync-bundled-docs.mjs')) {
  throw new Error('dist/assets/textforge-loader.js must not include bundled docs payload content');
}
...
for (const forbiddenStartupCopy of [
  ...
  if (distScriptJs.includes(forbiddenStartupCopy)) {
    throw new Error(`dist/assets/textforge-loader.js must not retain stale startup copy: ${forbiddenStartupCopy}`);
  }
}
...
for (const [label, source, pattern] of [
  ...
  ['dist/assets/textforge-bundled-docs.js', distDocsJs, /\b(?:fetch|XMLHttpRequest|WebSocket|EventSource|sendBeacon|import)\s*\(\s*["']https?:\/\//i],
]) {
