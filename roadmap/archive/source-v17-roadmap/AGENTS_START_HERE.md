...
TextForge is being rebuilt as a React-based, local-first, text-first workbench with a pnpm-workspace monorepo, package-level contribution boundaries, ITM as the canonical structural model, a virtual workspace, stable document identity badges, a readable overflow-safe workbench shell, Surface-based editors/viewers, restricted Lua automation, progressively implemented TF-MD Markdown/report generation, BPMN and enterprise architecture support, a reusable browser-envelope security profile, and a V17 dependency-based workpackage roadmap and a V16 backend-optional enterprise path based on provider contracts, revisions/changesets, manifests, one approved backend origin, and backend-enforced policy.
...
4. For WP5 onward, read `roadmap/workpackages/workpackage-register.md` before selecting work.
5. If `roadmap/grilling/phase-[PHASE-NUMBER]-grilling.md` exists for the active phase, read it as binding decision guidance after checking `RAPID.md` and the main roadmap entry.
6. Read `roadmap/02_phase_architecture_pnpm_matrix.md` for the active phase's architecture anchors and pnpm dependency commands.
7. Read `roadmap/03_package_dependency_activity_diagram.md` for cross-package phase sequencing context.
8. For Markdown work, read `roadmap/specs/textforge_markdown_profile.md` before implementing any TF-MD parsing, directive, include, block-registry, or rich-editor behavior.
9. For backend-optional, provider, settings, identity, persistence, service-folder, GitLab, AI, private/group space, or enterprise-container work, read `roadmap/textforge_backend_optional_architecture_whitepaper.md`, `roadmap/grilling/backend-grilling.md`, and `roadmap/packages/backend-optional.md` before implementation.
10. Inspect `git status`, current branch, recent commits, and package structure.
11. Determine the current phase/workpackage and the next incomplete step.
12. Check whether the roadmap still matches the repository reality.
13. Proceed only within the next coherent workpackage or phase slice.
...
## V17 workpackage planning rule

From WP5 onward, do not assume that numbered phase order is the only valid implementation order. Use the workpackage register and dependency reassessment first:

```text
roadmap/workpackages/workpackage-register.md
roadmap/workpackages/dependency-reassessment-v17.md
roadmap/workpackages/workpackage-dependency-map.md
```

The old phase entries remain the detailed scope source, but the register is the current planning/status layer. Optional adapters such as Entra SSO, GitLab, AI provider routing, OIDC/SAML, and future wrappers should not block generic contract/backend work unless the selected release scope explicitly requires them.

RAPID remains append-only history. The workpackage register is mutable current status. Update both when implementation state changes.
