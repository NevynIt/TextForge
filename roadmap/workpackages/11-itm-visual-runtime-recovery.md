...
|---|---|---|---|
| WP-VITM-01 | Visual ITM profile v1 | WP-ITM-01, WP-ITM-02 | Minimal constrained ITM profile for visual graph/tree/mindmap interchange. |
...
| WP-ITM-VRESOLVE-01 | Shared ITM visual target resolver | WP-VITM-01, WP-ITM-02, WP-ITM-VISUALS, WP-REPO-01 | **Needs grilling before implementation.** Resolves `%view`, `%viewpoint`, raw model fallback, filtered model, `render:` step, Visual ITM, diagnostics. |
...
|---|---|---|
| WP-VITM-01 | Further definition | Concrete Visual ITM v1 profile/spec and examples. |
| WP-ITM-VRESOLVE-01 | Grilling | Resolver contract for views, viewpoints, raw model fallback, `render:` precedence, renderer diagnostics, and no silent fallback. |
...
Not on this minimal chain: `WP-ITM-PUB-VISUAL-01`, `WP-MD-REPORT`, `WP-TABLES`, `WP-VITM-TRANSLATORS`, `WP-GRAPH-EDIT-VITM`, `WP-RES-03`, backend, identity, policy, and persistence work.
...
| `render:` respected | Existing ITM pipeline render step determines default renderer. |
...
| Search/selection sync works | Renderer supports old consumption workflow class where in scope. |
