...
- Workpackage ID: `WP-ITM-VTARGET-01`
...
Plain `.itm` open still goes to CodeMirror. `Open visuals...` now launches a surfaces-owned picker rendered in the workbench chrome, lists declared views/viewpoints/raw-model options, supports multi-select, and opens each choice as a separate session without flooding context menus.
...
Use `validation/checklists/workpackages/WP-ITM-VTARGET-01-itm-visual-target-picker.md` plus release-specific evidence.
