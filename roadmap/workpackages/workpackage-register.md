...
| WP-LUA-POWER-SESSION | New V18 follow-on | Lua self-escalation session and one-click recovery | Optional automation | Candidate | WP-LUA | High-trust local automation with scoped host-object access and fast recovery | Yes | No | Candidate follow-on to validated WP-LUA: session-scoped elevated host-object exposure, visible elevated-state badge, no auto-expire, and one-click recovery that restarts with a one-shot Lua-preload skip. |
...
- `WP-LUA-POWER-SESSION` is tracked as a candidate follow-on to `WP-LUA`; because its dependency is already validated, it should be shown as startable when included in dependency views, with actual execution order chosen one slice at a time.
