...
| WP-ITM-VISUALS | Phase 7 | ITM visual projections | Feature | Validated | WP-ITM-01, WP-05C | WP-BPMN-VISUAL, WP-GRAPH-EDIT | Yes | No | `@textforge/itm` now exposes tree/graph/mindmap/catalogue/matrix/report projection data plus Graphviz/Mermaid-mindmap source adapters, package-owned `.itm` projection surfaces, Markdown `itm-pub` publication output, and focused bundled test profiles for one-surface-at-a-time validation. |
...
| WP-LUA-POWER-SESSION | New V18 follow-on | Lua self-escalation session and one-click recovery | Optional automation | Validated | WP-LUA | High-trust local automation with scoped host-object access and fast recovery | Yes | No | Validated follow-on to `WP-LUA`: session-scoped power-session elevation ships approved `workspace`/`automation`/`surfaces` host objects, visible elevated-state console chrome, no auto-expire, and one-click recovery that restarts with a one-shot Lua-preload skip. |
...
- `WP-ITM-01`, `WP-ITM-02`, `WP-ITM-VISUALS`, and `WP-LUA` are validated; the next first-shippable ITM-adjacent slice now moves to `WP-MD-REPORT`.
...
- `WP-REPO-01`, `WP-ITM-02`, and `WP-ITM-VISUALS` are now validated; the main next recommended slice is `WP-MD-REPORT`, while `WP-RES-02`, `WP-ID-01`, `WP-SET-01`, `WP-BPMN-SEM`, `WP-ARCHIMATE-SEM`, and `WP-TABLES` also remain startable under the current dependency posture.
