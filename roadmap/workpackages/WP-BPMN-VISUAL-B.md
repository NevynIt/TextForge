...
- Workpackage ID: `WP-BPMN-VISUAL-B`
...
**Split from old WP-BPMN-VISUAL.** ITM `%view`/`%viewpoint` targets that declare a BPMN renderer now reuse the shared resolver path and open the BPMN viewer surface with source-file BPMN loading, DI application, provenance/diagnostics preservation, and no fallback to generic graph rendering.
...
Use `validation/checklists/workpackages/WP-BPMN-VISUAL-B-itm-bpmn-visual-target-integration.md` plus release-specific evidence.
